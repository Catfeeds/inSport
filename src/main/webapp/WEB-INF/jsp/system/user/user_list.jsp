									...
									<c:when test="${not empty userList}">
										<c:if test="${QX.cha == 1 }">
											<c:forEach items="${userList}" var="user" varStatus="vs">
												...
												<tr style="<c:if test="${user.STATUS == '1'}">color:red;</c:if>">
													...
													<td class="center">
														...
														<div class="hidden-sm hidden-xs btn-group">
															...
															<a class="btn btn-xs btn-primary" title="启用" onclick="openUser('${user.USER_ID}');">
																<i class="ace-icon fa fa-check-square-o bigger-120" title="启用"></i>
															</a>
															<c:if test="${QX.edit == 1 }">
															...
															<c:if test="${QX.del == 1 }">
																<a class="btn btn-danger btn-xs" title="停用用户" onclick="stopUser('${user.USER_ID }');">
																	<i class="ace-icon fa fa-exclamation-circle bigger-120" title="停用用户"></i>
																</a>
															</c:if>
														</div>
																...
																<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																	...
																	<c:if test="${QX.del == 1 }">
																		<li>
																			...
																			<a style="cursor:pointer;" onclick="stopUser('${user.USER_ID }');" class="tooltip-error" data-rel="tooltip" title="停用用户">
																	<span class="red">
																		<i class="ace-icon fa fa-trash-o bigger-120"></i>
																	</span>
																			</a>
...
<script type="text/javascript">
    ...
    function openUser(USER_ID){
        bootbox.confirm("确定要启用该用户吗?", function(result) {
            if(result) {
                $.ajax({
                    async: false,
                    cache: false,
    ...
    function stopUser(USER_ID){
        bootbox.confirm("确定要停用该用户吗?", function(result) {
            if(result) {
                $.ajax({
                    async: false,
                    cache: false,
