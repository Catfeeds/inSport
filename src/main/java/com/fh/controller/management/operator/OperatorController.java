package com.fh.controller.management.operator;
...
public class OperatorController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		pd.put("OPERATOR_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = operatorService.list(page);	//列出Operator列表
		mv.setViewName("management/operator/operator_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		List<PageData> listOperator = userService.listOperatorAll(pd);
		mv.setViewName("management/operator/operator_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Operator");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			operatorService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Operator到excel");
		...
		List<PageData> varOList = operatorService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("ONAME"));	    //1
